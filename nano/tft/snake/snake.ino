...
// 그래서 몸이 길어져도 속도가 전혀 느려지지 않는다.

#include "core.h"

#define EEPROM_ADDR       13

// 판 크기. 가로 16칸 * 세로 16칸이면 칸 번호가 0~255 라서 1바이트에 들어간다.
#define CELL              8
#define COLS              16
#define ROWS              16
#define FIELD_Y           (TFTH - ROWS * CELL)   // 32

// 몸 길이 상한. (칸이 256개지만 이 정도면 충분하고 RAM 도 아낀다)
#define MAX_LEN           180

#define START_LEN         4
#define START_STEP_MS     180
#define MIN_STEP_MS       70
#define STEP_DEC_MS       6     // 먹이 하나당 빨라지는 정도
...
void loop() {
  titleScreen("SNAKE", "JOYSTICK = TURN", "HOLD BUTTON = FAST", COLOR_LIME);
  showHighScore("SNAKE", EEPROM_ADDR);
