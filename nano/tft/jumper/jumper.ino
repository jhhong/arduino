...
#define EEPROM_ADDR       19

// 점수줄을 화면 맨 아래 10px 에 두고, 그 위쪽만 게임 화면으로 쓴다.
#define GAMEH             150
#define HUD_Y             (GAMEH + 2)

#define FP                16      // 고정소수점 배율 (1px = 16)
...
#define PLAT_W            26
#define PLAT_H            4
#define PLAT_GAP_MIN      22
#define PLAT_GAP_MAX      33

// 물리 (1/16 px 단위)
#define GRAVITY           2       // 프레임당 속도 증가
...
void playGame() {
  ...
  while (true) {
    ...
    // 좌우는 반대편으로 이어진다.
    if (playerX < -(int16_t)PLW * FP) {
      playerX = TFTW * FP;
    } else if (playerX > TFTW * FP) {
      playerX = -(int16_t)PLW * FP;
    }
...
void loop() {
  titleScreen("JUMPER", "JOYSTICK = MOVE", "BUTTON = HIGH JUMP", COLOR_CYAN);
  showHighScore("JUMPER", EEPROM_ADDR);
