...
void sleepNow()         // here we put the arduino to sleep
{
    ...
    attachInterrupt(3,wakeUpNow, LOW); // use interrupt 0 (pin 2) and run function
    ...
    sleep_disable();         // first thing after waking from sleep:
                             // disable sleep...
    detachInterrupt(3);      // disables interrupt 0 on pin 2 so the 
...
void loop() {
  ...
  if (soundValue > 200) {
     dbgSerial.println("arrived Sound Input");
     dbgSerial.println(soundValue);     
//    play();
    mode++;
    startTime = millis();
  }
